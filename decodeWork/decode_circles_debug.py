#!/usr/bin/env python3
"""
Décodeur avec détection de cercles et images de debug
"""
...
class SubtitleDecoderCirclesDebug:
    ...
    def detect_circles_with_debug(self, frame: np.ndarray, frame_count: int) -> List[Tuple[int, int]]:
        """
        Détection de cercles avec images de debug
        """
        detected_positions = []
            ...
            for (x, y, r) in circles:
                if self._is_red_circle(frame, x, y, r):
                    ...
                    if 0 <= grid_x < self.grid_width and 0 <= grid_y < self.grid_height:
                        hough_positions.append((grid_x, grid_y))
                        detected_positions.append((grid_x, grid_y))
                        
                        # Dessiner le cercle détecté en VERT
                        cv2.circle(debug_frame, (x, y), r, (0, 255, 0), 2)
                        cv2.circle(debug_frame, (x, y), 2, (0, 255, 0), -1)
                        
                        # Marquer la position dans la grille
                        center_x = grid_x * cell_width + cell_width // 2
                        center_y = grid_y * cell_height + cell_height // 2
                        cv2.putText(debug_frame, f"({grid_x},{grid_y})", 
                                  (center_x-20, center_y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        
        # Méthode 2: Détection par contours
        additional_positions = self._detect_by_contours_debug(frame, cell_width, cell_height, debug_frame)
        ...
        debug_filename = f"debug_frame_{frame_count:04d}.jpg"
        cv2.imwrite(debug_filename, debug_frame)
        
        return detected_positions
    
    def _detect_by_contours_debug(self, frame: np.ndarray, cell_width: int, cell_height: int, debug_frame: np.ndarray) -> List[Tuple[int, int]]:
        """
        Détection par contours avec debug
        """
        positions = []
                    ...
                    if circularity > 0.3:
                        ...
                        if M["m00"] != 0:
                            ...
                            if 0 <= grid_x < self.grid_width and 0 <= grid_y < self.grid_height:
                                ...
                                # Dessiner le contour détecté en BLEU
                                ...
                                center_x = grid_x * cell_width + cell_width // 2
                                center_y = grid_y * cell_height + cell_height // 2
                                cv2.putText(debug_frame, f"C({grid_x},{grid_y})", 
                                          (center_x+10, center_y+10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
    ...
    def decode_video(self, video_path: str, output_file: str = None, max_frames: int = None):
        """Décode la vidéo avec images de debug"""
        ...
        print("Décodage avec images de debug...")
        print("Images de debug sauvegardées: debug_frame_XXXX.jpg")
        ...
        while True:
            ...
            # Limiter le nombre de frames pour le debug
            if max_frames and frame_count >= max_frames:
                break
            
            current_time = frame_count / fps
            
            # Détecter les cercles avec debug
            detected_positions = self.detect_circles_with_debug(frame, frame_count)
        ...
        print(f"Images de debug: debug_frame_XXXX.jpg")
        
        return decoded_subtitles

def main():
    parser = argparse.ArgumentParser(description='Décoder avec images de debug')
    ...
    parser.add_argument('--max-frames', type=int, help='Nombre max de frames à traiter')
    ...
    decoder = SubtitleDecoderCirclesDebug(args.mapping)
