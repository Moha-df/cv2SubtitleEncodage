#!/usr/bin/env python3
"""
Décodeur amélioré pour les points visibles
"""
...
class SubtitleDecoderImproved:
    ...
    def detect_red_points_precise(self, frame: np.ndarray) -> List[Tuple[int, int]]:
        """
        Détection PRÉCISE des points rouges par centre de cercle
        """
        detected_positions = []
        ...
        # Détecter les contours des cercles rouges
        contours, _ = cv2.findContours(red_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        for contour in contours:
            # Vérifier si c'est un cercle de la bonne taille
            area = cv2.contourArea(contour)
            if area > 20:  # Filtre les petits bruits
    ...
    def decode_video(self, video_path: str, output_file: str = None):
        """Décode la vidéo avec détection améliorée"""
        ...
        print("Décodage amélioré des points ROUGES...")
        ...
        while True:
            ...
            # Détecter les points rouges avec précision
            detected_positions = self.detect_red_points_precise(frame)
            ...
            if detected_positions:
                ...
                if decoded_text and len(decoded_text) > 2:  # Filtrer les textes trop courts
        ...
        print(f"\nDécodage terminé!")
        print(f"Trouvé {len(decoded_subtitles)} sous-titres décodés valides")
        
        return decoded_subtitles

def main():
    parser = argparse.ArgumentParser(description='Décoder points ROUGES (version améliorée)')
    ...
    decoder = SubtitleDecoderImproved(args.mapping)
